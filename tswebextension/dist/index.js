...
import { RequestType, NetworkRuleOption, logger, StringRuleList, RuleConverter, RuleStorage, setConfiguration, CompatibilityTypes, Engine, Request, CosmeticResult, replaceAll, indexOfAny, NetworkRule } from '@adguard/tsurlfilter';
import { z } from 'zod';
 ...
 */
const isOwnUrl = (referrerUrl) => {
    return referrerUrl.indexOf(browser.runtime.getURL('')) === 0;
};
const isHttpOrWsRequest = (url) => {
...
class StealthService {
     ...
     */
    processRequestHeaders(requestUrl, requestType, requestHeaders) {
...
const buildExtendedCssScriptText = (extendedCssStylesheets) => {
    return `(function() {
...
class Frame {
    constructor(data) {
        this.requests = new RequestContextStorage();
        this.url = data.url;
...
class TabContext {
    ...
    reloadTabFrameData(url) {
        var _a;
        const previousUrl = (_a = this.frames.get(MAIN_FRAME_ID)) === null || _a === void 0 ? void 0 : _a.url;
        this.frames.clear();
...
class TabsApi {
    ...
    setTabFrameRule(tabId, frameRule) {
    ...
    recordRequestFrame(tabId, frameId, url, requestType) {
        const tabContext = this.context.get(tabId);
        if (!tabContext) {
            return;
        }
    ...
    updateTabContextData(tabId, changeInfo) {
...
class FrameRequestService {
     ...
     */
    static search({ tabId, frameId, requestUrl, requestType, }) {
     ...
     */
    static prepareSearchParams(requestUrl, tabId, frameId) {
...
class CosmeticApi {
    ...
    static injectExtCss(extCssText, tabId, frameId = 0) {
    ...
    static getExtCssText(cosmeticResult, collectingCosmeticRulesHits = false) {
        const { elementHiding, CSS } = cosmeticResult;
    ...
    static getScriptText(cosmeticResult) {
    ...
    static getFrameExtCssText(frameUrl, tabId, frameId) {
        ...
        const cosmeticResult = requestContext === null || requestContext === void 0 ? void 0 : requestContext.cosmeticResult;
        if (!cosmeticResult) {
            return;
        }
 ...
 */
CosmeticApi.buildStyleSheetWithHits = (elemhideRules, injectRules) => {
...
function getRequestInitiatorTag(requestType) {
    switch (requestType) {
        case RequestType.Subdocument:
            return ["iframe" /* IFRAME */, "frame" /* FRAME */];
...
function getRequestType(resourceType) {
    switch (resourceType) {
        ...
        case 'csp_report':
            return {
                contentType: "CSP_REPORT" /* CSP_REPORT */,
...
class RequestBlockingApi {
    static processShouldCollapse(tabId, url, referrerUrl, requestType) {
    ...
    static isDocumentBlockingRule(requestRule) {
        return !!requestRule
            && !requestRule.isAllowlist()
    ...
    static getBlockedResponseByRule(requestRule, requestType) {
 ...
 */
class RequestEvent {
    constructor(event, handler, filter, extraInfoSpec) {
        this.listeners = [];
...
const onBeforeRequest = new RequestEvent(browser.webRequest.onBeforeRequest, (details) => {
    const { requestId, frameId, tabId, timeStamp, } = details;
    const context = requestContextStorage.record(requestId, {
        state: "BEFORE_REQUEST" /* BEFORE_REQUEST */,
        requestId,
        frameId,
        tabId,
        timestamp: timeStamp,
...
const onBeforeSendHeaders = new RequestEvent(browser.webRequest.onBeforeSendHeaders, (details) => {
    const { requestId, timeStamp, requestHeaders } = details;
    const context = requestContextStorage.update(requestId, {
        state: "BEFORE_SEND_HEADERS" /* BEFORE_SEND_HEADERS */,
        timestamp: timeStamp,
        requestHeaders,
    });
    return { details, context };
}, { urls: ['<all_urls>'] }, onBeforeSendHeadersOptions);
new RequestEvent(browser.webRequest.onSendHeaders, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
        state: "SEND_HEADERS" /* SEND_HEADERS */,
        timestamp: timeStamp,
    });
    return { details, context };
}, { urls: ['<all_urls>'] });
...
const onHeadersReceived = new RequestEvent(browser.webRequest.onHeadersReceived, (details) => {
    const { requestId, responseHeaders, statusCode, } = details;
    const context = requestContextStorage.update(requestId, {
        state: "HEADERS_RECEIVED" /* HEADERS_RECEIVED */,
        responseHeaders,
        statusCode,
    });
    return { details, context };
}, { urls: ['<all_urls>'] }, onHeadersReceivedOptions);
new RequestEvent(browser.webRequest.onAuthRequired, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
...
new RequestEvent(browser.webRequest.onBeforeRedirect, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
        state: "BEFORE_REDIRECT" /* BEFORE_REDIRECT */,
        timestamp: timeStamp,
    });
    return { details, context };
}, { urls: ['<all_urls>'] });
const onResponseStarted = new RequestEvent(browser.webRequest.onResponseStarted, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
...
const onCompleted = new RequestEvent(browser.webRequest.onCompleted, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
...
const onErrorOccurred = new RequestEvent(browser.webRequest.onErrorOccurred, (details) => {
    const { requestId, timeStamp } = details;
    const context = requestContextStorage.update(requestId, {
        state: "ERROR" /* ERROR */,
        timestamp: timeStamp,
...
class StealthApi {
    ...
    start(configuration) {
        return __awaiter(this, void 0, void 0, function* () {
            this.configuration = Object.assign({}, configuration.settings.stealth);
            this.engine = new StealthService(this.configuration);
            onBeforeSendHeaders.addListener(this.onBeforeSendHeaders);
            if (StealthApi.canBlockWebRTC()) {
                let isPermissionsGranted = false;
                try {
                    isPermissionsGranted = yield browser.permissions.contains(StealthApi.PRIVACY_PERMISSIONS);
                }
                catch (e) {
    ...
    onBeforeSendHeaders({ details }) {
        ...
        const stealthActions = this.engine.processRequestHeaders(context.requestUrl, context.requestType, details.requestHeaders);
        if (stealthActions > 0) {
    ...
    static canApplyStealthActionsToContext(context) {
...
const stealthApi = new StealthApi(defaultFilteringLog);
    ...
    startEngine(configuration) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (allowlist.length > 0) {
                lists.push(new StringRuleList(ALLOWLIST_FILTER_ID, allowlist.map((domain) => {
...
class HeadersService {
    ...
    onBeforeSendHeaders(context) {
        const { requestHeaders, matchingResult, tabId, requestUrl, requestId, } = context;
        if (!requestHeaders
            || !matchingResult
            || !requestUrl) {
            return false;
    ...
    onHeadersReceived(context) {
        const { responseHeaders, matchingResult, tabId, requestUrl, requestId, } = context;
        if (!responseHeaders
            || !matchingResult
            || !requestUrl) {
            return false;
...
class CookieFiltering {
     ...
     * @private
     */
    applyRulesToCookieHeaders(context) {
    ...
    applyRules(context) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const promises = cookies.map((cookie) => __awaiter(this, void 0, void 0, function* () {
                yield this.applyRulesToCookie(cookie, cookieRules, tabId);
            }));
    ...
    applyRulesToCookie(cookie, cookieRules, tabId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (mRules.length > 0) {
                const appliedRules = CookieFiltering.applyRuleToBrowserCookie(cookie, mRules);
                if (appliedRules.length > 0) {
                    if (yield this.browserCookieApi.modifyCookie(cookie)) {
                        appliedRules.forEach((r) => {
                            this.filteringLog.addCookieEvent({
                                tabId,
                                cookieName: cookie.name,
...
class ContentFiltering {
    ...
    static onBeforeRequest(requestId) {
...
class CspService {
    ...
    static onHeadersReceived(context) {
        const { matchingResult, responseHeaders, requestId, } = context;
        if (!matchingResult) {
            return false;
        }
        const cspHeaders = [];
...
class WebRequestApi {
    static start() {
        ...
        onBeforeRequest.addListener(WebRequestApi.onBeforeRequest);
        onBeforeSendHeaders.addListener(WebRequestApi.onBeforeSendHeaders);
        onHeadersReceived.addListener(WebRequestApi.onHeadersReceived);
        onResponseStarted.addListener(WebRequestApi.onResponseStarted);
        onErrorOccurred.addListener(WebRequestApi.onErrorOccurred);
        onCompleted.addListener(WebRequestApi.onCompleted);
        // browser.webNavigation Events
        browser.webNavigation.onCommitted.addListener(WebRequestApi.onCommitted);
    }
    static stop() {
        onBeforeRequest.removeListener(WebRequestApi.onBeforeRequest);
        onBeforeSendHeaders.removeListener(WebRequestApi.onBeforeSendHeaders);
        onHeadersReceived.removeListener(WebRequestApi.onHeadersReceived);
        onResponseStarted.removeListener(WebRequestApi.onResponseStarted);
        onErrorOccurred.removeListener(WebRequestApi.onErrorOccurred);
        onCompleted.removeListener(WebRequestApi.onCompleted);
        browser.webNavigation.onCommitted.removeListener(WebRequestApi.onCommitted);
    }
    static onBeforeRequest(data) {
        const { details } = data;
        const { requestId, type, frameId, tabId, parentFrameId, originUrl, initiator, method, } = details;
        let { url } = details;
        /**
    ...
    static onErrorOccurred({ details, }) {
        ...
        const frame = tabsApi.getTabFrame(tabId, frameId);
        if (frame === null || frame === void 0 ? void 0 : frame.injection) {
    ...
    static recordFrameInjection(context) {
        const { cosmeticResult, tabId, frameId, } = context;
        if (!cosmeticResult) {
            return;
        }
    ...
    static injectJsScript(tabId, frameId) {
        var _a;
        const frame = tabsApi.getTabFrame(tabId, frameId);
        if ((_a = frame === null || frame === void 0 ? void 0 : frame.injection) === null || _a === void 0 ? void 0 : _a.jsScriptText) {
...
class MessagesApi {
     ...
     * @param listener
     */
    addAssistantCreateRuleListener(listener) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (type) {
                ...
                case MessageType.GET_EXTENDED_CSS: {
                    return this.handleGetExtendedCssMessage(sender, message.payload);
                }
    ...
    handleGetExtendedCssMessage(sender, payload) {
        var _a;
        if (!payload || !((_a = sender === null || sender === void 0 ? void 0 : sender.tab) === null || _a === void 0 ? void 0 : _a.id)) {
            return false;
        }
...
class TsWebExtension {
    ...
    start(configuration) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield stealthApi.start(configuration);
            WebRequestApi.start();
            this.isStarted = true;
     ...
     */
    static mergeConfiguration(configuration, changes) {
...
export { MESSAGE_HANDLER_NAME, TsWebExtension };
